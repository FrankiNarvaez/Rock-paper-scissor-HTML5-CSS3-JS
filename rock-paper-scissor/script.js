...
playerVsMachineButton.addEventListener('click', function() {
    let attemptsValue = attemptsInput.value.trim();
    if (!validateAttempts(attemptsValue)) {
        alert('Por favor, ingresa un número entero positivo de intentos.');
        return;
    }
    attempts = parseInt(attemptsValue);
    selectPlayerModeSection.style.display = 'none';
    playerVsMachineSection.style.display = 'block';
    startPlayerVsMachine();
});

function startPlayerVsMachine() {
    resetGame();
    // Agregar listeners a los botones del jugador
    document.getElementById('rock').addEventListener('click', () => playPlayerVsMachine('rock'));
    document.getElementById('paper').addEventListener('click', () => playPlayerVsMachine('paper'));
    document.getElementById('scissor').addEventListener('click', () => playPlayerVsMachine('scissor'));
}

function playPlayerVsMachine(choice) {
    playerChoice = choice;
    ...
    setTimeout(() => {
        ...
        setTimeout(() => {
            ...
            updateScorePvM();
            ResultGameRound(result, playerChoice, machineChoice);
...
function ResultGameRound(result, choicePlayer1, choiceMachine) {
    let message = '';
    if (result === 'player') {
...
function returnToMainScreen() {
    ...
    // Mostrar el inicio principal del juego
    selectPlayerModeSection.style.display = 'flex';
